...
type MockChartDAO_MinSellPrice2 struct {
	dao.ChartDAO
	mock.Mock
}

type MockChartDAO_MinSellPrice2_2 struct {
	dao.ChartDAO
	mock.Mock
}
...
func TestDefaultTradingStrategy_minSellPrice_LastPriceGreater(t *testing.T) {
	...
	chart := new(MockChart)
	chartDAO := new(MockChartDAO_MinSellPrice2)
	...
	tradingFee := chartService.GetExchange().GetTradingFee()
	minPrice := strategy.minSellPrice(tradingFee)
	assert.Equal(t, float64(18000), strategy.lastTrade.Price)
	assert.Equal(t, float64(10000), chartService.GetData().Price)
	assert.Equal(t, 0.025, chartService.GetExchange().GetTradingFee()) // 10000 * .025 = 250
...
func TestDefaultTradingStrategy_minSellPrice_NoProfitPercent(t *testing.T) {
	...
	assert.Equal(t, float64(18000), strategy.lastTrade.Price)
	assert.Equal(t, float64(10000), chartService.GetData().Price)
	assert.Equal(t, 0.025, chartService.GetExchange().GetTradingFee()) // 18000 + 500 = 18500 * 0.025 = 462.5
...
func TestDefaultTradingStrategy_minSellPrice_DoesntIncludeTax(t *testing.T) {
	...
	assert.Equal(t, float64(18000), strategy.lastTrade.Price)
	assert.Equal(t, float64(10000), chartService.GetData().Price)
	assert.Equal(t, 0.025, chartService.GetExchange().GetTradingFee()) // 18000 + 500 = 18500 * .025 = 462.5
	assert.Equal(t, float64(0), strategy.config.profitMarginMinPercent)
	assert.Equal(t, float64(500), strategy.config.profitMarginMin)
...
func TestDefaultTradingStrategy_minSellPrice_IncludesTax(t *testing.T) {
	...
	chartService := new(MockChartService)
	chart := new(MockChart)
	chartDAO := new(MockChartDAO_MinSellPrice2_2)
	chartDAO.On("GetLastTrade", chart).Return(chartDAO.GetLastTrade(chart)).Once()
	strategy := CreateDefaultTradingStrategy(ctx, chart, chartDAO, new(MockProfitDAO), &DefaultTradingStrategyConfig{
		rsiOverSold:            30,
		rsiOverBought:          70,
	...
	assert.Equal(t, float64(10000), chartService.GetData().Price)
	assert.Equal(t, 0.025, chartService.GetExchange().GetTradingFee()) // 9500 * .025 = 237.5
	assert.Equal(t, float64(0), strategy.config.profitMarginMinPercent)
	assert.Equal(t, float64(500), strategy.config.profitMarginMin)
	assert.Equal(t, .20, strategy.config.tax)  // 500 * .20 = 100
...
func (mdao *MockChartDAO_MinSellPrice2) GetLastTrade(autoTradeCoin dao.IChart) *dao.Trade {
...
func (mdao *MockChartDAO_MinSellPrice2) Save(dao dao.IChart) {}

func (mdao *MockChartDAO_MinSellPrice2_2) GetLastTrade(autoTradeCoin dao.IChart) *dao.Trade {
	mdao.Called(autoTradeCoin)
	return &dao.Trade{
		Date:     time.Now().AddDate(0, 0, -20),
		Type:     "sell",
...
func (mdao *MockChartDAO_MinSellPrice2_2) Save(dao dao.IChart) {}
