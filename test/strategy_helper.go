...
func (h *StrategyTestHelper) CreateIndicators() map[string]common.FinancialIndicator {
	rsi := indicators.NewRelativeStrengthIndex(h.CreateCandles())
	bollinger := indicators.NewBollingerBands(h.CreateCandles())
	macd := indicators.NewMovingAverageConvergenceDivergence(h.CreateCandles())
	return map[string]common.FinancialIndicator{
