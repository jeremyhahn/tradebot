...
func (eth *EthService) DeleteAccount(passphrase string) error {
	...
	if user == nil {
		eth.ctx.GetLogger().Error("[EthereumService.DeleteAccount] No user context")
...
func (eth *EthService) GetAccounts() ([]accounts.Account, error) {
	user := eth.ctx.GetUser()
	if user == nil {
		eth.ctx.GetLogger().Error("[EthereumService.GetAccounts] No user context")
...
func (eth *EthService) Login(username, password string) (common.UserContext, error) {
	userEntity, err := eth.userDAO.GetByName(username)
	if err != nil {
		return nil, err
	}
	eth.ctx.GetLogger().Debugf("[EthereumService.Login] userEntity: %+v", userEntity)
	err = eth.Authenticate(userEntity.GetEtherbase(), password)
	if err != nil {
		eth.ctx.GetLogger().Errorf("[EhtereumService.Login] %s", err.Error())
		return nil, err
	}
...
func (eth *EthService) Register(username, password string) error {
	...
	if err != nil && err.Error() != "record not found" {
		eth.ctx.GetLogger().Errorf("[EthereumService.Register] %s", err.Error())
...
func (eth *EthService) GetTokenBalance(contract string, wallet string) (common.EthereumToken, error) {
	...
	if err != nil {
		eth.ctx.GetLogger().Errorf("Failed to instantiate a Token contract: %v", err)
		return nil, err
	}

	address := ethcommon.HexToAddress(wallet)
	if err != nil {
		eth.ctx.GetLogger().Errorf("Failed hex address: "+wallet, err)
		return nil, err
	}

	ethAmount, err := eth.client.BalanceAt(context.Background(), address, nil)
	if err != nil {
		eth.ctx.GetLogger().Errorf("Failed to get ethereum balance from contract %s. Error: %s", address, err.Error())
		return nil, err
	}

	balance, err := token.BalanceOf(nil, address)
	if err != nil {
		eth.ctx.GetLogger().Errorf("Failed to get balance from contract %s. Error: %s", contract, err.Error())
		return nil, err
	}
	symbol, err := token.Symbol(nil)
	if err != nil {
		eth.ctx.GetLogger().Errorf("Failed to get symbol from contract %s. Error: %s", contract, err.Error())
		return nil, err
	}
	tokenDecimals, err := token.Decimals(nil)
	if err != nil {
		eth.ctx.GetLogger().Errorf("Failed to get decimals from contract %s. Error: %s", contract, err.Error())
		return nil, err
	}
	name, err := token.Name(nil)
	if err != nil {
		eth.ctx.GetLogger().Errorf("Failed to retrieve token name from contract %s. Error: %s", contract, err.Error())
		return nil, err
	}
